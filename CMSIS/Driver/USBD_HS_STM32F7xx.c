 ...
 * Project:      USB High-Speed Device Driver for ST STM32F7xx
 * --------------------------------------------------------------------------
...
static void USBD_Reset (void) {
  ...
  OTG->DIEPTXF6   = ((OTG_RX_FIFO_SIZE  + OTG_TX0_FIFO_SIZE + OTG_TX1_FIFO_SIZE +
                      OTG_TX2_FIFO_SIZE + OTG_TX3_FIFO_SIZE + OTG_TX4_FIFO_SIZE +
                      OTG_TX5_FIFO_SIZE)/ 4U) |
                    ((OTG_TX6_FIFO_SIZE / 4U) << OTG_HS_DIEPTXFx_INEPTXFD_POS);

  OTG->DIEPTXF7   = ((OTG_RX_FIFO_SIZE  + OTG_TX0_FIFO_SIZE + OTG_TX1_FIFO_SIZE +
                      OTG_TX2_FIFO_SIZE + OTG_TX3_FIFO_SIZE + OTG_TX4_FIFO_SIZE +
                      OTG_TX5_FIFO_SIZE + OTG_TX6_FIFO_SIZE)/ 4U) |
                    ((OTG_TX7_FIFO_SIZE / 4U) << OTG_HS_DIEPTXFx_INEPTXFD_POS);
}
...
static int32_t USBD_PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      setup_received =  0U;                             // Reset variables
      memset((void *)(&usbd_state), 0, sizeof(usbd_state));
      memset((void *)(ep),          0, sizeof(ep));
    ...
    case ARM_POWER_FULL:
...
#if ((USBD_HS_VBUS_DETECT == 1) && (defined(MX_USB_OTG_HS_VBUS_Pin) || defined(MX_USB_OTG_HS_ULPI_D7_Pin)))
      OTG->GCCFG    |=  OTG_HS_GCCFG_VBDEN;             // Enable VBUS sensing
      OTG->GOTGCTL  &= ~OTG_HS_GOTGCTL_BVALOEN;         // B-peripheral session valid override disable
#else
      OTG->GCCFG    &= ~OTG_HS_GCCFG_VBDEN;             // Disable VBUS sensing
      OTG->GOTGCTL  |=  OTG_HS_GOTGCTL_BVALOEN;         // B-peripheral session valid override enable
      OTG->GOTGCTL  |=  OTG_HS_GOTGCTL_BVALOVAL;        // B-peripheral session valid override value
#endif
      ...
      OTG->GAHBCFG  |= (OTG_HS_GAHBCFG_GINTMSK  |       // Enable interrupts
