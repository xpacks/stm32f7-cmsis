 ...
 * Project:      USB High/Full-Speed Host Driver for ST STM32F7xx
 * --------------------------------------------------------------------------
 ...
 *   USBH1_MANAGE_CACHE: specifies if USB data is in cacheable area
 *                      (0 = data in non-cacheable area,
 *                       1 = data in cacheable area)
 *     - default value : 0 = data in non-cacheable area
...
static bool USBH_HW_StartTransfer (PIPE_t *ptr_pipe, OTG_HS_HC *ptr_ch) {
  ...
  uint8_t   out;
#else                                           // If DMA is used
#if ((USBH1_MANAGE_CACHE == 1U) && (__DCACHE_PRESENT == 1U))
  uint8_t   out;
#endif
#endif
  ...
  hcchar       &= (OTG_HS_HCCHARx_ODDFRM    |   // Keep ODDFRM
                   ...
                   OTG_HS_HCCHARx_MCNT_MSK  |   // Keep MCNT
...
//                   OTG_HS_HCINTMSKx_ACKM    | // After ACK there must be other relevant interrupt so ACK is ignorred
  ...
  if ((SCB->CCR & SCB_CCR_DC_Msk) != 0U) {      // If Data Cache is enabled
    if ((out != 0U) && (num_to_transfer > 0U)) {
      SCB_CleanDCache_by_Addr ((uint32_t *)((uint32_t)(ptr_pipe->data + ptr_pipe->num_transferred_total) & (~0x1FU)), num_to_transfer + 31U);
    }
  }
#endif
...
static ARM_USBH_PIPE_HANDLE USBH_PipeCreate (uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size, uint8_t  ep_interval) {
  ...
  switch (ep_type) {
    ...
    case ARM_USB_ENDPOINT_INTERRUPT:
      ...
      ptr_ch->HCCHAR |= OTG_HS_HCCHARx_MCNT((((ep_max_packet_size >> 11) + 1U) & 3U));
      break;
  }
...
#if ((USBH1_MANAGE_CACHE == 1U) && (__DCACHE_PRESENT == 1U))
              if ((SCB->CCR & SCB_CCR_DC_Msk) != 0U) {  // If Data Cache is enabled
                SCB_InvalidateDCache_by_Addr ((uint32_t *)((uint32_t)ptr_pipe->data & (~0x1FU)), ptr_pipe->num_transferred_total + 31U);
              }
#endif
            }
          } else {                                      // If OUT pipe
